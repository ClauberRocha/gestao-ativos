...
export default function Home({ onExit }: { onExit?: () => void }) {
  ...
  const openNewAsset = () => { if (!user) { setAuthOpen(true); toast.info("Entre para cadastrar um ativo"); return; } setEditingAsset(null); setForm(emptyForm); setSheetOpen(true); };
  const openAsset = (asset: Asset) => { setEditingAsset(asset); setForm({ patrimonio: asset.patrimonio, descricao: asset.descricao, numero_serie: asset.numero_serie, conta_cliente: asset.conta_cliente ?? "", local: asset.local ?? "", status: asset.status, conservacao: asset.conservacao ?? "", valor_aquisicao: asset.valor_aquisicao ?? null, observacoes: asset.observacoes ?? "" }); setSheetOpen(true); };
  ...
  const saveAsset = async () => {
    ...
    try {
      ...
      toast.success(editingAsset ? "Ativo atualizado" : "Ativo cadastrado", { description: `${form.patrimonio} foi salvo no inventário.` });
      setSheetOpen(false);
      await loadAssets();
  ...
  return <DashboardLayout onOpenAuth={() => setAuthOpen(true)} onExit={onExit}><div className="mx-auto max-w-[1480px] space-y-6">
    ...
    <div className="flex flex-col gap-2 border-t border-border/60 pt-4 text-[11px] text-muted-foreground sm:flex-row sm:items-center sm:justify-between"><span>MR Pay · Gestão de Ativos · v1.0.0.1</span><span className="flex items-center gap-1.5"><Check className="size-3 text-emerald-600" /> {user ? `${profile?.role === "admin" ? "Admin" : "Operador"} conectado` : "Modo demonstração"}</span></div>
  </div><Sheet open={sheetOpen} onOpenChange={setSheetOpen}><AssetForm asset={editingAsset} form={form} setForm={setForm} isNew={!editingAsset} isAdmin={isAdmin} saving={saving} onSave={() => void saveAsset()} onDelete={() => void deleteAsset()} onClose={() => setSheetOpen(false)} demoMode={usingDemoData} /></Sheet><AuthDialog open={authOpen} onOpenChange={setAuthOpen} />{authLoading && <div className="pointer-events-none fixed bottom-4 right-4 rounded-full border border-border/70 bg-card/90 px-3 py-1.5 text-[11px] text-muted-foreground shadow-lg">Verificando sessão...</div>}</DashboardLayout>;
}
