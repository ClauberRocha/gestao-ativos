...
export type AssetSortKey = "patrimonio" | "status" | "conservacao" | "conta_cliente" | "updated_at";
  ...
  const loadAssets = useCallback(async () => {
    ...
    if (!user || !isSupabaseConfigured) {
      ...
      const sorted = [...filtered].sort((a, b) => {
        const left = sortKey === "updated_at" ? new Date(a.updated_at ?? a.created_at ?? 0).getTime() : String(a[sortKey] ?? "").toLowerCase();
        const right = sortKey === "updated_at" ? new Date(b.updated_at ?? b.created_at ?? 0).getTime() : String(b[sortKey] ?? "").toLowerCase();
