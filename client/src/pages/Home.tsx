...
export default function Home() {
  ...
  const openAsset = (asset: Asset) => { setEditingAsset(asset); setForm({ patrimonio: asset.patrimonio, descricao: asset.descricao, numero_serie: asset.numero_serie, conta_cliente: asset.conta_cliente ?? "", local: asset.local ?? "", status: asset.status, conservacao: asset.conservacao ?? "", valor_aquisicao: asset.valor_aquisicao ?? null, observacoes: asset.observacoes ?? "" }); setSaveCompleted(false); setSheetOpen(true); };
  ...
  const saveAsset = async () => {
    ...
    try {
      ...
      toast.success("Ativo Salvo com sucesso", { description: `${form.patrimonio} foi salvo no inventário.` });
