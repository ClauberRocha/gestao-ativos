...
import { ArrowDownToLine, ArrowDownUp, ArrowRight, Boxes, Check, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, CircleAlert, Database, LayoutDashboard, Loader2, LockKeyhole, LogIn, MapPin, PackageCheck, Plus, Search, ShieldCheck, SlidersHorizontal, Sparkles, Trash2, UserRound, Users, X } from "lucide-react";
...
export type AssetSortKey = "patrimonio" | "descricao" | "status" | "conta_cliente" | "local" | "conservacao" | "numero_serie" | "updated_at" | "valor_aquisicao";
export type AssetFilters = { query: string; status: StatusFilter; conservacao: string; contaCliente: string; createdFrom: string; createdTo: string; modifiedFrom: string; modifiedTo: string };

export function filterAssetRows(rows: Asset[], filters: AssetFilters) {
  const { query, status, conservacao, contaCliente, createdFrom, createdTo, modifiedFrom, modifiedTo } = filters;
  const normalizedQuery = query.trim().toLowerCase();
  return rows.filter((asset) => {
    const matchesQuery = !normalizedQuery || asset.patrimonio.toLowerCase().includes(normalizedQuery) || asset.descricao.toLowerCase().includes(normalizedQuery) || asset.numero_serie.toLowerCase().includes(normalizedQuery);
    ...
    const createdValue = new Date(asset.created_at ?? 0).getTime();
    const modifiedValue = new Date(asset.updated_at ?? asset.created_at ?? 0).getTime();
    const matchesCreatedFrom = !createdFrom || createdValue >= new Date(`${createdFrom}T00:00:00`).getTime();
    const matchesCreatedTo = !createdTo || createdValue <= new Date(`${createdTo}T23:59:59.999`).getTime();
    const matchesModifiedFrom = !modifiedFrom || modifiedValue >= new Date(`${modifiedFrom}T00:00:00`).getTime();
    const matchesModifiedTo = !modifiedTo || modifiedValue <= new Date(`${modifiedTo}T23:59:59.999`).getTime();
    return matchesQuery && matchesStatus && matchesConservation && matchesClient && matchesCreatedFrom && matchesCreatedTo && matchesModifiedFrom && matchesModifiedTo;
  });
}
...
function SortIndicator({ active, direction }: { active: boolean; direction: "asc" | "desc" }) {
  if (!active) return <ArrowDownUp aria-hidden="true" className="size-3 text-muted-foreground/70" />;
  return direction === "asc"
    ? <ChevronUp aria-hidden="true" className="size-3 text-primary" />
    : <ChevronDown aria-hidden="true" className="size-3 text-primary" />;
}
...
export function AuthDialog({ open, onOpenChange, initialMode = "login", allowSignup = false }: { open: boolean; onOpenChange: (open: boolean) => void; initialMode?: "login" | "signup"; allowSignup?: boolean }) {
  const { signIn, signUp } = useSupabaseAuth();
  const [mode, setMode] = useState<"login" | "signup">(allowSignup ? initialMode : "login");
  useEffect(() => { if (open) setMode(allowSignup ? initialMode : "login"); }, [open, initialMode, allowSignup]);
...
export default function Home() {
  ...
  const loadAssets = useCallback(async () => {
    ...
    if (!user || !isSupabaseConfigured) {
      const filtered = filterAssetRows(sampleAssets, { query, status, conservacao, contaCliente, createdFrom, createdTo, modifiedFrom, modifiedTo });
      setTotal(filtered.length);
      const sorted = [...filtered].sort((a, b) => {
        const left = sortKey === "updated_at" ? new Date(a.updated_at ?? a.created_at ?? 0).getTime() : sortKey === "valor_aquisicao" ? Number(a.valor_aquisicao ?? 0) : String(a[sortKey] ?? "").toLowerCase();
        const right = sortKey === "updated_at" ? new Date(b.updated_at ?? b.created_at ?? 0).getTime() : sortKey === "valor_aquisicao" ? Number(b.valor_aquisicao ?? 0) : String(b[sortKey] ?? "").toLowerCase();
        const comparison = left < right ? -1 : left > right ? 1 : 0;
        return sortDirection === "asc" ? comparison : -comparison;
      });
      setAssets(sorted.slice((page - 1) * pageSize, page * pageSize));
    ...
    try {
      let request = supabase.from("assets_inventory").select("*", { count: "exact" }).order(sortKey, { ascending: sortDirection === "asc" });
      ...
      if (createdFrom) request = request.gte("created_at", `${createdFrom}T00:00:00`);
      if (createdTo) request = request.lte("created_at", `${createdTo}T23:59:59.999`);
      if (modifiedFrom) request = request.gte("updated_at", `${modifiedFrom}T00:00:00`);
      if (modifiedTo) request = request.lte("updated_at", `${modifiedTo}T23:59:59.999`);
      if (query.trim()) {
        const safeQuery = query.trim().replace(/[%,()]/g, "");
        request = request.or(`patrimonio.ilike.%${safeQuery}%,descricao.ilike.%${safeQuery}%,numero_serie.ilike.%${safeQuery}%`);
      }
      const from = (page - 1) * pageSize;
      const [{ data, error, count }, summary] = await Promise.all([request.range(from, from + pageSize - 1), supabase.from("assets_inventory").select("status")]);
      ...
      if (!user) {
        const filtered = filterAssetRows(sampleAssets, { query, status, conservacao, contaCliente, createdFrom, createdTo, modifiedFrom, modifiedTo });
        setTotal(filtered.length);
        setAssets(filtered.slice((page - 1) * pageSize, page * pageSize));
        setUsingDemoData(true);
      } else {
        setAssets([]);
        setTotal(0);
  ...
  }, [page, pageSize, query, status, conservacao, contaCliente, createdFrom, createdTo, modifiedFrom, modifiedTo, sortKey, sortDirection, user]);

  useEffect(() => { const timer = window.setTimeout(() => void loadAssets(), 180); return () => window.clearTimeout(timer); }, [loadAssets]);

  if (!authLoading && !user) {
    return <><InitialAuthScreen onOpenAuth={() => { setAuthMode("login"); setAuthOpen(true); }} /><AuthDialog open={authOpen} onOpenChange={setAuthOpen} initialMode={authMode} allowSignup={false} /></>;
  }
  ...
  const currentFilters: AssetFilters = { query, status, conservacao, contaCliente, createdFrom, createdTo, modifiedFrom, modifiedTo };
  const applyFilters = (filters: AssetFilters) => { setQuery(filters.query); setStatus(filters.status); setConservacao(filters.conservacao); setContaCliente(filters.contaCliente); setCreatedFrom(filters.createdFrom); setCreatedTo(filters.createdTo); setModifiedFrom(filters.modifiedFrom); setModifiedTo(filters.modifiedTo); setPage(1); };
  const resetFilters = () => applyFilters({ query: "", status: "Todos", conservacao: "", contaCliente: "", createdFrom: "", createdTo: "", modifiedFrom: "", modifiedTo: "" });
  const saveCurrentFilters = () => { localStorage.setItem(FILTER_STORAGE_KEY, JSON.stringify(currentFilters)); setSavedFilters(currentFilters); toast.success("Filtros salvos"); };
  ...
  const openUserCreate = () => { const mode = getUserCreationMode(Boolean(user), profile?.role); setAuthMode(mode); setAuthOpen(true); if (mode === "login") toast.info("Entre com um perfil administrador para criar usuários"); };
  const deleteAsset = async (assetToDelete?: Asset) => {
    const target = assetToDelete ?? editingAsset;
    if (!target || !isAdmin || !user || !isSupabaseConfigured) return;
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(target.id)) {
      toast.error("Registro indisponível", { description: "Atualize a lista para carregar o id real do ativo." });
      return;
    }
    if (!window.confirm(`Excluir o ativo ${target.patrimonio}? Esta ação não pode ser desfeita.`)) return;
    ...
    toast.success("Ativo excluído", { description: `${target.patrimonio} foi removido do inventário.` });
    if (editingAsset?.id === target.id) setSheetOpen(false);
    await loadAssets();
  };
  ...
  const importAssets = async (rows: ImportRow[], fileName: string, extraHeaders: string[]) => {
    ...
    try {
      const imported = await replaceAssetDatabase(supabase, rows, fileName);
      toast.success("Base substituída com segurança", { description: `${imported} registros importados. Campos extras preservados: ${extraHeaders.length}.` });
    } catch (error) { toast.error("Importação não concluída", { description: error instanceof Error ? error.message : "Verifique a migration e tente novamente." }); return; }
    await loadAssets();
  };
  ...
  const exportAssets = async () => {
    ...
    if (modifiedFrom) request = request.gte("updated_at", `${modifiedFrom}T00:00:00`);
    if (modifiedTo) request = request.lte("updated_at", `${modifiedTo}T23:59:59.999`);
    if (query.trim()) { const safeQuery = query.trim().replace(/[%,()]/g, ""); request = request.or(`patrimonio.ilike.%${safeQuery}%,descricao.ilike.%${safeQuery}%,numero_serie.ilike.%${safeQuery}%`); }
    ...
    await recordAudit(user.id, "export", { count: rows.length, filters: currentFilters });
  ...
  const clearAssets = async () => {
    ...
    try {
      const removed = await clearAssetDatabase(supabase);
      toast.success("Base atual limpa", { description: `${removed} registros removidos. Perfis e logs foram preservados.` });
    } catch (error) { toast.error("A base não foi limpa", { description: error instanceof Error ? error.message : "Verifique a migration e tente novamente." }); return; }
    await loadAssets();
  };
  ...
  return <DashboardLayout onOpenAuth={() => { setAuthMode("login"); setAuthOpen(true); }} onOpenUserCreate={openUserCreate} isAdmin={isAdmin} onOverviewClick={() => { setShowLogs(false); window.history.replaceState(null, "", getMenuHash("Visão geral")); const overview = overviewRef.current; if (overview && typeof overview.scrollIntoView === "function") overview.scrollIntoView({ behavior: "smooth", block: "start" }); }} onInventoryClick={() => { setShowLogs(false); window.history.replaceState(null, "", getMenuHash("Inventário")); const inventory = document.getElementById("inventario"); if (inventory && typeof inventory.scrollIntoView === "function") inventory.scrollIntoView({ behavior: "smooth", block: "start" }); }} onLogsClick={() => { setShowLogs(true); window.history.replaceState(null, "", "#logs"); window.setTimeout(() => document.getElementById("logs")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}><div className="mx-auto max-w-[1480px] space-y-6">{showLogs ? <AuditLogPanel logs={auditLogs} loading={auditLoading} onRefresh={() => void loadAuditLogs()} /> : <div className="contents">
    ...
    <section className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_10px_40px_-32px_rgba(15,23,42,0.55)]"><div className="flex flex-col gap-3 border-b border-border/70 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5"><div className="relative min-w-0 flex-1 sm:max-w-[480px]"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input data-testid="asset-search-input" aria-label="Buscar por patrimônio ou número de série" value={query} onChange={(event) => updateQuery(event.target.value)} placeholder="Buscar por nome, patrimônio ou número de série..." className="h-10 rounded-xl border-border/80 bg-muted/35 pl-9 pr-9 text-sm shadow-none focus-visible:bg-background" />{query && <button type="button" onClick={() => updateQuery("")} aria-label="Limpar busca" className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground transition hover:text-foreground"><X className="size-3.5" /></button>}</div><div className="flex items-center gap-2"><div className="hidden items-center gap-1.5 text-xs font-medium text-muted-foreground sm:flex"><SlidersHorizontal className="size-3.5" /> Filtrar por</div><Select value={status} onValueChange={(value) => updateStatus(value as StatusFilter)}><SelectTrigger aria-label="Filtrar por status" className="h-10 w-full rounded-xl bg-muted/35 text-xs sm:w-[160px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Todos">Todos os status</SelectItem>{ASSET_STATUSES.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div></div><div className="grid gap-2 border-t border-border/60 bg-muted/15 p-4 sm:grid-cols-2 lg:grid-cols-4"><Select value={conservacao || "Todos"} onValueChange={(value) => { setConservacao(value === "Todos" ? "" : value); setPage(1); }}><SelectTrigger aria-label="Filtrar por conservação" className="h-9 rounded-lg bg-background text-xs"><SelectValue placeholder="Conservação" /></SelectTrigger><SelectContent><SelectItem value="Todos">Todas as conservações</SelectItem>{["Ótimo", "Novo", "Bom", "Regular", "Ruim"].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select><select aria-label="Filtrar por conta cliente" value={contaCliente} onChange={(event) => { setContaCliente(event.target.value); setPage(1); }} className="h-9 rounded-lg border border-input bg-background px-3 text-xs"><option value="">Todas as contas cliente</option>{Array.from(new Set(assets.map((asset) => asset.conta_cliente).filter(Boolean))).sort().map((value) => <option key={value} value={value as string}>{value}</option>)}</select><Input aria-label="Cadastro desde" type="date" value={createdFrom} onChange={(event) => { setCreatedFrom(event.target.value); setPage(1); }} placeholder="Cadastro desde" className="h-9 rounded-lg bg-background text-xs" /><Input aria-label="Cadastro até" type="date" value={createdTo} onChange={(event) => { setCreatedTo(event.target.value); setPage(1); }} placeholder="Cadastro até" className="h-9 rounded-lg bg-background text-xs" /><Input aria-label="Modificação desde" type="date" value={modifiedFrom} onChange={(event) => { setModifiedFrom(event.target.value); setPage(1); }} placeholder="Modificação desde" className="h-9 rounded-lg bg-background text-xs" /><Input aria-label="Modificação até" type="date" value={modifiedTo} onChange={(event) => { setModifiedTo(event.target.value); setPage(1); }} placeholder="Modificação até" className="h-9 rounded-lg bg-background text-xs" /><div className="flex flex-wrap items-center gap-2 sm:col-span-2 lg:col-span-4"><Button type="button" variant="outline" size="sm" onClick={resetFilters} className="h-8 rounded-lg text-xs">Limpar filtros</Button><Button type="button" variant="outline" size="sm" onClick={saveCurrentFilters} className="h-8 rounded-lg text-xs">Salvar filtros</Button>{savedFilters && <Button type="button" variant="ghost" size="sm" onClick={() => applyFilters(savedFilters)} className="h-8 rounded-lg text-xs">Reutilizar filtros salvos</Button>}</div></div>{connectionError && <div className="flex flex-col gap-2 border-b border-amber-200 bg-amber-50 px-5 py-3 text-xs text-amber-800 sm:flex-row sm:items-center sm:justify-between"><span><strong>Modo demonstração:</strong> a tabela Supabase não respondeu. Execute <code className="rounded bg-amber-100 px-1">supabase/schema.sql</code> no SQL Editor para carregar os dados reais.</span><button type="button" onClick={() => setConnectionError(null)} className="font-semibold underline underline-offset-2">Ocultar</button></div>}<div className="relative overflow-x-auto"><div className="pointer-events-none absolute inset-x-0 top-0 z-10 h-0.5 overflow-hidden bg-primary/15">{loading && <div className="asset-loading-bar h-full w-1/3 rounded-full bg-primary" />}</div><Table className="min-w-[1300px]"><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="w-[152px]"><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("patrimonio"); setSortDirection(sortKey === "patrimonio" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por patrimônio">Patrimônio <SortIndicator active={sortKey === "patrimonio"} direction={sortDirection} /></button></TableHead><TableHead><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("descricao"); setSortDirection(sortKey === "descricao" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por descrição">Descrição <SortIndicator active={sortKey === "descricao"} direction={sortDirection} /></button></TableHead><TableHead><div className="flex items-center gap-2"><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("conta_cliente"); setSortDirection(sortKey === "conta_cliente" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por conta cliente">Cliente <SortIndicator active={sortKey === "conta_cliente"} direction={sortDirection} /></button><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("local"); setSortDirection(sortKey === "local" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por local">Local <SortIndicator active={sortKey === "local"} direction={sortDirection} /></button></div></TableHead><TableHead><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("status"); setSortDirection(sortKey === "status" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por status">Status <SortIndicator active={sortKey === "status"} direction={sortDirection} /></button></TableHead><TableHead><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("conservacao"); setSortDirection(sortKey === "conservacao" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por conservação">Conservação <SortIndicator active={sortKey === "conservacao"} direction={sortDirection} /></button></TableHead><TableHead><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("numero_serie"); setSortDirection(sortKey === "numero_serie" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por número de série">Número de série <SortIndicator active={sortKey === "numero_serie"} direction={sortDirection} /></button></TableHead><TableHead className="text-right"><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("updated_at"); setSortDirection(sortKey === "updated_at" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por data de atualização">Atualização <SortIndicator active={sortKey === "updated_at"} direction={sortDirection} /></button></TableHead><TableHead className="text-right"><button type="button" className="inline-flex items-center gap-1 font-medium" onClick={() => { setSortKey("valor_aquisicao"); setSortDirection(sortKey === "valor_aquisicao" && sortDirection === "asc" ? "desc" : "asc"); setPage(1); }} aria-label="Ordenar por valor de aquisição">Aquisição <SortIndicator active={sortKey === "valor_aquisicao"} direction={sortDirection} /></button></TableHead>{isAdmin && <TableHead className="w-14 text-right">Ações</TableHead>}</TableRow></TableHeader><TableBody>{loading ? <TableLoading /> : assets.length === 0 ? <TableRow><TableCell colSpan={isAdmin ? 9 : 8} className="h-48 text-center"><div className="mx-auto flex max-w-xs flex-col items-center"><div className="mb-3 flex size-10 items-center justify-center rounded-2xl bg-muted"><Search className="size-4 text-muted-foreground" /></div><p className="text-sm font-semibold">Nenhum ativo encontrado</p><p className="mt-1 text-xs text-muted-foreground">Ajuste os termos de busca ou limpe os filtros.</p><Button variant="ghost" size="sm" onClick={resetFilters} className="mt-3 text-xs">Limpar filtros</Button></div></TableCell></TableRow> : assets.map((asset) => <TableRow key={asset.id} className="group cursor-pointer" tabIndex={0} onClick={() => openAsset(asset)} onKeyDown={(event) => { if (event.key === "Enter") openAsset(asset); }}><TableCell><span className="font-mono text-xs font-semibold tracking-tight text-foreground">{asset.patrimonio}</span></TableCell><TableCell><div className="max-w-[220px]"><p className="truncate text-sm font-medium">{asset.descricao}</p><p className="mt-0.5 text-[11px] text-muted-foreground">ID · {asset.id.slice(0, 8).toUpperCase()}</p></div></TableCell><TableCell><p className="text-xs font-semibold">{asset.conta_cliente || "—"}</p><p className="mt-0.5 flex items-center gap-1 text-[11px] text-muted-foreground"><MapPin className="size-3" /> {asset.local || "Local não informado"}</p></TableCell><TableCell><StatusBadge status={asset.status} /></TableCell><TableCell><span className="text-xs text-muted-foreground">{asset.conservacao || "—"}</span></TableCell><TableCell><span className="font-mono text-[11px] text-muted-foreground">{asset.numero_serie}</span></TableCell><TableCell className="text-right"><span className="text-xs text-muted-foreground">{asset.updated_at ? new Date(asset.updated_at).toLocaleDateString("pt-BR") : "—"}</span></TableCell><TableCell className="text-right"><span className="text-xs font-medium text-muted-foreground">{isAdmin ? formatCurrency(asset.valor_aquisicao) : "••••••"}</span></TableCell>{isAdmin && <TableCell className="text-right"><Button type="button" variant="ghost" size="icon" className="size-8 text-destructive hover:bg-red-50 hover:text-destructive" aria-label={`Excluir ${asset.patrimonio}`} title="Excluir ativo" onClick={(event) => { event.stopPropagation(); void deleteAsset(asset); }}><Trash2 className="size-3.5" /></Button></TableCell>}</TableRow>)}</TableBody></Table></div><div className="flex flex-col gap-3 border-t border-border/70 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div className="flex items-center gap-2 text-xs text-muted-foreground"><span>Exibir</span><select aria-label="Quantidade de ativos por página" value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value) as (typeof PAGE_SIZE_OPTIONS)[number]); setPage(1); }} className="h-8 rounded-lg border border-input bg-background px-2 text-xs">{PAGE_SIZE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select><span>por página</span></div><p className="text-xs text-muted-foreground">Mostrando <span className="font-medium text-foreground">{visibleRangeStart}–{visibleRangeEnd}</span> de <span className="font-medium text-foreground">{total}</span> ativos</p><div className="flex items-center gap-1.5"><Button variant="outline" size="icon" aria-label="Página anterior" disabled={page === 1 || loading} onClick={() => setPage((current) => Math.max(1, current - 1))} className="size-8 rounded-lg bg-background"><ChevronLeft className="size-3.5" /></Button><span className="min-w-16 text-center text-xs font-medium text-muted-foreground">Página {page} / {totalPages}</span><Button variant="outline" size="icon" aria-label="Próxima página" disabled={page >= totalPages || loading} onClick={() => setPage((current) => Math.min(totalPages, current + 1))} className="size-8 rounded-lg bg-background"><ChevronRight className="size-3.5" /></Button></div></div></section>
